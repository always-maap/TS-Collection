const createFunctionWithTimeout = (callback: () => void, opt_timeout = 1000) => {
  let called = false;
  const raceCallback = () => {
    if (!called) {
      called = true;
...
export function trackCustomEvent({
  ...
  if (typeof window !== `undefined` && (window as any).gtag) {
    ...
    if (hitCallback && typeof hitCallback === `function`) {
      trackingEventOptions.event_callback = createFunctionWithTimeout(hitCallback, callbackTimeout);
    }
